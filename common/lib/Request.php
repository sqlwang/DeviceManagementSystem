...
class Request {
    ...
    protected function parseRequest() {
        if ($this->method == 'PUT') {   // <-- Have to jump through hoops to get PUT data
			...
			$this->requesetParams = 'http://'.$_SERVER['HTTP_HOST'].'?'.$raw;
        } else if($this->method == 'GET'){
        	$this->requesetParams = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
        }else {
            ...
            if (isset($_REQUEST['data'])) {
				...
				$this->requesetParams =  'http://'.$_SERVER['HTTP_HOST'].'?'.$_REQUEST['data'];
            } else {
                $raw  = '';
                $httpContent = fopen('php://input', 'r');
                while ($kb = fread($httpContent, 1024)) {
